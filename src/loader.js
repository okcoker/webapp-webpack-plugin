...
module.exports = function (content) {
  ...
  const outputPath = query.outputPath ? trailingSlash(query.outputPath) : null;
  // Generate icons
  return favicons(content, Object.assign(query.options, { path: url.resolve(path, prefix) }))
    .then(({ html: tags, images, files }) => {
      const assets = [...images, ...files].map(({ name, contents }) => ({ name: outputPath ? outputPath + name : prefix + name, contents }));
